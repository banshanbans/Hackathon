...
W5 已完成代码、合同、API/H5/iOS 测试、Simulator build 和 Debug Fixture 自动化范围。动作提示已前置到 W4 相机 Overlay；人物框与目标框 IoU 达到 80%并稳定 1.2 秒后直接自动倒数三秒，随后按任务模式执行三张照片连拍或 6 秒无声短视频。后续仍包含本地候选推荐、用户确认单张 JPEG、明确上传与 External AI 同意、评价、唯一复拍建议、第二轮重新对齐，以及 iOS/H5 两端结果恢复和三图对比。
...
- W4 达到 80% IoU 并稳定后保留同一 Camera Session，直接进入三秒自动倒计时。倒计时期间 IoU 低于 70%，或单人、完整度、构图、姿势、相机状态失效时会取消并重新对齐；`manual` 必须二次确认并持续标记为未经 Vision 验证。
...
- 第一轮 Coaching 顶部并排展示“参考图”和“第一拍”，随后展示作品就绪度和进度条；唯一问题、唯一建议及“其他都保持不变”的语义保持不变。
- 最终结果使用可滑动分页画廊和“参考 / 第一拍 / 调整后”分段选择；只有实际存在的轮次才进入映射，参考图失效时显示诚实占位且不阻断已拍照片。

## 用户可见行为

- 动作说明、拍摄模式、IoU 和三秒倒计时均显示在相机页；正常路径不再要求“准备动作”“我已就位”点击，语音只使用本地固定短句。
...
- H5 最终页在桌面宽度下并排展示参考、第一拍和第二拍，在移动端使用横向吸附；Fixture 缺少真实成片时不生成替代图片。
...
自动测试覆盖 Claim Token 与同意门禁、真实/Fixture 媒体边界、两轮顺序与幂等、Provider 恢复、1.1 多模态输入、敏感字段过滤、候选评分与间隔、80%自动触发、70%取消重试、工作存储、上传 URL 过期、冷启动恢复、照片/短视频/降级 UI 流程，以及 H5 跨端轮询和刷新恢复。

2026-07-22 增量验证中，`SoloShotTests` 全量通过，包含零轮、第一轮、第二轮及多余轮次的对比项映射；`W5CaptureUITests` 通过，覆盖第一轮参考图/第一拍/就绪度和最终三项切换。Simulator 构建随上述 XCTest 成功完成。
...
- 预置参考图已从错误的 XcodeGen target-level `resources` 配置迁移到显式 `buildPhase: resources`；Simulator `.app` 已核对包含四张 `*-thumb.webp`，避免演示任务显示“参考图暂不可用”。
- 短视频抽帧不再请求精确的 `0s` 和视频结束时刻，改为从六个时间区间的中心取样，并允许前后 `200ms` 容差。该改动规避真实 H.264 MOV 在尾边界没有可解码帧时整轮失败的问题。
- `AVAssetWriter` 的输入创建、启动和 append 失败会被保留并在停止录制时返回，不再统一沉默成无上下文失败；错误页同时展示实际错误文本，并继续提供“三张连拍”恢复入口。
- XCTest 已覆盖抽帧数量、顺序和边界；Simulator 单元测试与资源/首页 UI 测试通过。真实后置相机仍必须在指定 iPhone 上复测 5–8 秒录制、方向、候选质量和连续十次成功率，Simulator 不能关闭该验收项。
