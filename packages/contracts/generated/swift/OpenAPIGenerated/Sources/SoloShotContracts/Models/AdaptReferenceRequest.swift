//
// AdaptReferenceRequest.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation

public struct AdaptReferenceRequest: Sendable, Codable, Hashable {

    public enum SchemaVersion: String, Sendable, Codable, CaseIterable {
        case _10 = "1.0"
    }
    public static let sessionIdRule = StringRule(minLength: nil, maxLength: nil, pattern: "/^ss_[A-Za-z0-9_-]+$/")
    public static let referenceIdRule = StringRule(minLength: nil, maxLength: nil, pattern: "/^ref_[A-Za-z0-9_-]+$/")
    public static let sceneAssetIdRule = StringRule(minLength: nil, maxLength: nil, pattern: "/^media_[A-Za-z0-9_-]+$/")
    public var schemaVersion: SchemaVersion
    public var sessionId: String
    public var referenceId: String
    public var sceneAssetId: String

    public init(schemaVersion: SchemaVersion, sessionId: String, referenceId: String, sceneAssetId: String) {
        self.schemaVersion = schemaVersion
        self.sessionId = sessionId
        self.referenceId = referenceId
        self.sceneAssetId = sceneAssetId
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case schemaVersion = "schema_version"
        case sessionId = "session_id"
        case referenceId = "reference_id"
        case sceneAssetId = "scene_asset_id"
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(schemaVersion, forKey: .schemaVersion)
        try container.encode(sessionId, forKey: .sessionId)
        try container.encode(referenceId, forKey: .referenceId)
        try container.encode(sceneAssetId, forKey: .sceneAssetId)
    }
}

