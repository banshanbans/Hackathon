//
// AnalyzeReferenceRequest.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation

public struct AnalyzeReferenceRequest: Sendable, Codable, Hashable {

    public enum SchemaVersion: String, Sendable, Codable, CaseIterable {
        case _10 = "1.0"
    }
    public static let sessionIdRule = StringRule(minLength: nil, maxLength: nil, pattern: "/^ss_[A-Za-z0-9_-]+$/")
    public var schemaVersion: SchemaVersion
    public var sessionId: String
    public var referenceAsset: ReferenceAsset

    public init(schemaVersion: SchemaVersion, sessionId: String, referenceAsset: ReferenceAsset) {
        self.schemaVersion = schemaVersion
        self.sessionId = sessionId
        self.referenceAsset = referenceAsset
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case schemaVersion = "schema_version"
        case sessionId = "session_id"
        case referenceAsset = "reference_asset"
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(schemaVersion, forKey: .schemaVersion)
        try container.encode(sessionId, forKey: .sessionId)
        try container.encode(referenceAsset, forKey: .referenceAsset)
    }
}

