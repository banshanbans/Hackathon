...
参考图下载后，iOS 使用 `selected_box` 在设备内选择目标人物实例、验证全身可见性并提取版本化 2D 轮廓。相机实时循环在单人场景下使用快速人物分割生成实时轮廓；蒙版、轮廓坐标和预览帧不保存、不上传。正常压力目标 6–10Hz，Serious 降为 3–5Hz，Critical 沿用手动确认降级。
...
生产 UI 显示参考人物橙色虚线、实时人物青色实线、唯一指令、轮廓接近度、倒计时和录制状态；达到现有自动拍摄条件后两条轮廓变绿。参考轮廓不可用时才回退目标框、头部圆和脚底线。人物框、头脚辅助、关节点、构图 IoU 和性能数据只在 Debug 模式显示。

轮廓接近度使用固定低分辨率蒙版的 Dice 系数并做时间平滑。它是软评分，不参与首版 `ready_to_capture` 或倒计时取消；构图、完整入镜和受控姿势仍由现有规则判断。
